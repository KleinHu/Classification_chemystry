...
#压缩图片,把图片压缩成64*64的
def resize_img():
	dirs = os.listdir("split_pic//3")
	for filename in dirs:
		im = tf.gfile.FastGFile("split_pic//3//{}".format(filename), 'rb').read()
...
#压缩图片,把图片压缩成64*64的
def resize_img_test():
	dirs = os.listdir("pictures//test_pic")
	for filename in dirs:
		im = tf.gfile.FastGFile("pictures//test_pic//{}".format(filename), 'rb').read()
		# print("正在处理第%d张照片"%counter)
		with tf.Session() as sess:
			img_data = tf.image.decode_jpeg(im)
			image_float = tf.image.convert_image_dtype(img_data, tf.float32)
			...
			resized_im = resized.eval()
			# new_mat = np.asarray(resized_im).reshape(1, 64, 64, 3)
			scipy.misc.imsave("pictures//test_resized_pic//{}".format(filename),resized_im)
